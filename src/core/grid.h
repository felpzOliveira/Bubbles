...
static const int kDefaultMaxParticles=1000000;
template<typename T, typename U, typename Q>
class ContinuousParticleSetBuilder{
    ...
    ContinuousParticleSetBuilder(int maxParticles=kDefaultMaxParticles,
...
template<typename T, typename U, typename Q>
class ProgressiveParticleSetBuilder{
    public:
    ...
    Grid<T, U, Q> *mappedDomain;
    std::set<unsigned int> mappedCellSet;
    std::map<unsigned int, MappedCell<T>> mappedPositions;

    ProgressiveParticleSetBuilder(int maxParticles=kDefaultMaxParticles){
        maxNumOfParticles = Max(1, maxParticles);
        particleSet = cudaAllocateVx(ParticleSet<T>, 1);
        particleSet->SetSize(maxNumOfParticles);
        int pSize = MaximumParticlesPerBucket;
    ...
    void MapGrid(Grid<T, U, Q> *grid){
        ...
        for(int i = 0; i < count; i++){
            ...
            if(ret.first != mappedCellSet.end()){
                ...
                int size = cell->GetChainLength();
                AssureA(size > 0,
               "Called for MapGrid but grid is inconsistent, missing first distribution?");
                for(int j = 0; j < size; j++){
                    T pj = particleSet->GetParticlePosition(pChain->pId);
                    pos.push_back(pj);
                    pChain = pChain->next;
                ...
                mappedPositions[h] = MappedCell<T>{0, pos};
            }
        }
        ...
        for(it = mappedCellSet.begin(); it != mappedCellSet.end(); it++){
            unsigned int h = *it;
            std::vector<T> &pos = mappedPositions[h].pos;
            Cell<Q> *cell = mappedDomain->GetCell(h);
            AssureA(pos.size() == cell->GetChainLength(),
                    "Inconsistent grid mapping detected, invalid first distribution?");
        }
    ...
    void MapGridEmit(const std::function<T(const T &)> velocity, Q bounds, Float d=0.02){
        std::set<unsigned int>::iterator it;
        int numNewParticles = 0;
        for(it = mappedCellSet.begin(); it != mappedCellSet.end(); it++){
            unsigned int h = *it;
            MappedCell<T> &mCell = mappedPositions[h];
            Cell<Q> *cell = mappedDomain->GetCell(h);

            if(!Overlaps(cell->bounds, bounds))
                continue;
            ...
            int size = cell->GetChainLength();
            if(size >= MaximumParticlesPerBucket) continue;
            int toInsert = Min(MaximumParticlesPerBucket - size, mCell.pos.size());
            mCell.emitted = 1;
        ...
        if(numNewParticles > 0)
            ...
            printf("[Warning] Called emit but no particle was generated");
    }

    void MapGridEmitToOther(ProgressiveParticleSetBuilder<T, U, Q> *other,
                            const std::function<T(const T &)> velocity, Q bounds,
                            Float d=0.02)
    {
        std::set<unsigned int>::iterator it;
        int numNewParticles = 0;
...
typedef ProgressiveParticleSetBuilder<vec2f, vec2ui, Bounds2f> ProgressiveParticleSetBuilder2;
typedef ProgressiveParticleSetBuilder<vec3f, vec3ui, Bounds3f> ProgressiveParticleSetBuilder3;
