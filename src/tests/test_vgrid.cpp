...
void test_macgrid_solver(){
    printf("===== Test MACGrid2 Solver\n");
    CudaMemoryManagerStart(__FUNCTION__);
    vec2ui res(300);
    Float hx = 1.f / (Float)res.x;
    Float density = 0.1;

    TestSolver solver(res, hx);
    vec2f p(0.45, 0.2);
    vec2f len(0.1, 0.01);
    Float d_in = 1.f;
    Float v_in = 3.f;
    ...
    auto *u = solver.macGrid->U_ptr();
    auto *v = solver.macGrid->V_ptr();
    auto *d = solver.density;

    MACVelocityGrid2 tmpMac;

    tmpMac.Init(vec2f(0.f), res, hx, false);
    solver.matPtr = &matGrid;
    ...
    int frame = 0;
    while(true){
        AddInflow<MonotonicCatmull>(solver.density, p, len, d_in);
        AddInflow<MonotonicCatmull>(solver.macGrid->V_ptr(), p, len, v_in);
        ...
        if(0){
            ...
            params.fluidCells = &fluidCells;
            params.materialGrid = &matGrid;
            params.velocityField = solver.macGrid;
            ...
            for(unsigned int idx = 0; idx < fluidCells.size(); idx++){
                g = fluidCells[idx];
                pGrid.Set(g, pressures[idx]);
            }
        }else{
            PressureSolverEigen2 pSolver;
            pSolver.BuildSolver(res.x * res.y, hx, res);
            ...
            pSolver.UpdateFull(200, pSolver.p, density, timestep, 1,
                               u_sample, v_sample, d_sample);
            for(int i = 0; i < res.x; i++){
                for(int j = 0; j < res.y; j++){
                    pGrid.Set(i, j, pSolver.p[i + j * res.x]);
                }
            }
        }

        {
            apply_pressure(solver, &tmpMac, &pGrid, timestep);
        }

        {
            GridAdvectionSolver2 advectionSolver;
            advectionSolver.Advect(solver.macGrid, d, timestep);
            advectionSolver.Advect(solver.macGrid, u, timestep);
            advectionSolver.Advect(solver.macGrid, v, timestep);
